...
void insert(int key, int value)
{
    ...
    {
        if (array[i].data != NULL && array[i].data->key == key)
        {
        ...
        if (i == index) //jika sudah mengecek satu -satu index sampai balik lagi ke index penuh
        {
            ...
            max *= 2;
            array = (struct hastable_item*)realloc(array, max * sizeof(struct hashtable_item));
            init_array();
            ...
            for (int j = 0; j < max / 2; j++)
            {
                if (array[j].flag == 1)
                {
                    insert(array[j].data->key, array[j].data->value);
                    free(array[j].data);
                }
            }
